...
mod test_lib {
    ...
    fn expect_text_tag_tokens() {
        ...
        // Text (with cloning using to_owned)
        let text = res.get(&3)
            .unwrap()
            .to_owned()
            .text
        ...
        // However if use to_owned value will be move, hence we'll need to clone this value for later usage
